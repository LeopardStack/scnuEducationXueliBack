...
public abstract class OfficeAutomationHandler {
    ...
    protected Boolean success(Long approvalId, Long stepId, Date date) {
        ...
        List<ApprovalStepRecordWithStepInformation> approvalStepRecordWithStepInformation = approvalStepRecordService.selectApprovalRecordWithApprovalRecordId(approvalId);
        if (CollUtil.isEmpty(approvalStepRecordWithStepInformation)) {
            throw new BusinessException("获取审核步骤记录失败");
        }
        Map<Integer, List<ApprovalStepRecordWithStepInformation>> stepOrder2InformationMap = approvalStepRecordWithStepInformation.stream()
                .collect(Collectors.groupingBy(ApprovalStepRecordWithStepInformation::getStepOrder));
        ...
        for (Integer order : stepOrder2InformationMap.keySet()) {
            ...
            List<ApprovalStepRecordWithStepInformation> approvalStepRecordWithStepInformations = stepOrder2InformationMap.get(order);
            if (CollUtil.isEmpty(approvalStepRecordWithStepInformations)) {
                continue;
            }
            Set<String> statusSet = approvalStepRecordWithStepInformations.stream().map(ApprovalStepRecordWithStepInformation::getStatus)
        ...
        if (Objects.nonNull(orderId)) {
            List<ApprovalStepRecordWithStepInformation> approvalStepRecordWithStepInformations = stepOrder2InformationMap.get(orderId);
            nextStepId = approvalStepRecordWithStepInformations.get(0).getStepId();
        } else {
